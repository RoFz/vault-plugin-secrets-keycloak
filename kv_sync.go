...
// kvSyncRequest holds the parameters for a single KV v2 sync write.
...
// writeKVSecret PATCHes key=password into the KV v2 secret at
...
func writeKVSecret(ctx context.Context, r kvSyncRequest) (string, error) {
	...
	kvPath := fmt.Sprintf("%s/data/%s", r.mountPath, r.secretPath)
...
func (b *keycloakBackend) syncKVForResponse(ctx context.Context, s logical.Storage, kvKey, password string, logFields ...interface{}) []string {
	...
	warning, err := writeKVSecret(ctx, kvSyncRequest{
		vaultAddr:     addr,
